...
RUN pkg update && pkg install -y \
    ...
    py312-onnx py312-onnxruntime \
    git-lite gmake pkgconf FreeBSD-bmake \
    py312-scikit-build-core py312-build py312-installer \
...
# chmod: ocijail 0.6.0 leaks umask(077) into the build, so a port built here
# installs unreadable by the runtime user (PermissionError on import as bsd).
RUN ports-build --cleanup math/py-ml-dtypes lang/python312 && \
    chmod -R a+rX /usr/local/lib/python3.12/site-packages/ml_dtypes && \
